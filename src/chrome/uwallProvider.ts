...
class UwallProvider extends EventEmitter {
  isUwall = true;
  isMetaMask = true;
    ...
    switch (method) {
      ...
      case "eth_accounts": {
        let accountPromise = new Promise((resolve, reject) => {
          // send message to content_script (inject.ts) to fetch the user address
          window.postMessage(
            {
        ...
        const res = await accountPromise;
        return [res];
      }
            ...
            (e: any) => {
              ...
              switch (e.data.type) {
                case "switchEthereumChain": {
                  ...
                  ((window as Window).ethereum as UwallProvider).setChainId(chainId, rpcUrl);
                  // remove this listener as we already have a listener for "message" and don't want duplicates
                  controller.abort();
...
window.addEventListener("message", (e: any) => {
  ...
  switch (e.data.type) {
    case "init": {
      ...
      const rpcUrl = e.data.msg.rpcUrl as string;
      try {
        const uwallProvider = new UwallProvider(chainId, rpcUrl, address);
        (window as Window).ethereum = uwallProvider;
      } catch (error) {
        console.log(error);
      }
    ...
    case "setAddress": {
      const address = e.data.msg.address as string;
      ((window as Window).ethereum as UwallProvider).setAddress(address);
      break;
    }
    case "setChainId": {
      const chainId = e.data.msg.chainId as number;
    ...
    case "close": {
      window.sessionStorage.setItem("isUwallActive", "false");
    }
  }
});
